...
static t_config get_default_config()
{
    ...
    {
        config.silent_mode_dialog_choices[g_io_service.string_to_wstring(pair.first)] = std::to_wstring(pair.second);
    }
...
{
    std::string str = g_io_service.wstring_to_string(field_name);
...
static void handle_config_value(mINI::INIStructure& ini, const std::wstring& field_name, const int32_t is_reading, std::wstring& value)
{
    ...
    if (is_reading)
    {
        ...
        value = g_io_service.string_to_wstring(ini[FLAT_FIELD_KEY][key]);
    }
    else
    {
        ini[FLAT_FIELD_KEY][key] = g_io_service.wstring_to_string(value);
    }
}
...
static void handle_config_value(mINI::INIStructure& ini, const std::wstring& field_name, const int32_t is_reading, std::vector<std::wstring>& value)
{
    ...
    if (is_reading)
    {
        ...
        {
            value.push_back(g_io_service.string_to_wstring(ini[key][std::to_string(i)]));
        }
    }
    else
    {
        ...
        {
            ini[key][std::to_string(i)] = g_io_service.wstring_to_string(value[i]);
        }
    }
}
...
static void handle_config_value(mINI::INIStructure& ini, const std::wstring& field_name, const int32_t is_reading, std::map<std::wstring, std::wstring>& value)
{
    ...
    if (is_reading)
    {
        ...
        for (auto& pair : map)
        {
            value[g_io_service.string_to_wstring(pair.first)] = g_io_service.string_to_wstring(pair.second);
        }
    }
    else
    {
        ...
        for (auto& pair : value)
        {
            ini[key][g_io_service.wstring_to_string(pair.first)] = g_io_service.wstring_to_string(pair.second);
        }
    }
}
...
static void handle_config_value(mINI::INIStructure& ini, const std::wstring& field_name, const int32_t is_reading, std::map<std::wstring, Hotkey::t_hotkey>& value)
{
    ...
    const auto prefix = g_io_service.wstring_to_string(std::format(L"{}_", field_name));
    ...
    if (is_reading)
    {
        for (const auto& pair : ini)
        {
            ...
            value[g_io_service.string_to_wstring(action_path)] = hotkey;
        }
    }
    else
    {
        ...
        {
            const auto action_key = prefix + g_io_service.wstring_to_string(action_path);
...
static void config_patch(t_config& cfg)
{
    ...
    for (const auto& pair : DIALOG_SILENT_MODE_CHOICES)
    {
        const auto key = g_io_service.string_to_wstring(pair.first);
...
static void migrate_config(t_config& config, const mINI::INIStructure& ini)
{
    const auto migrate_hotkey = [&](const std::string& old_section_name, std::wstring action) {
        ...
        g_view_logger->info(L"[Config] Migrating {} -> {} ({})", g_io_service.string_to_wstring(old_section_name), action, hotkey.to_wstring());
