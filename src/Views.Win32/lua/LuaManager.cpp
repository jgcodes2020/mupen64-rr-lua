...
{
    const auto message = g_io_service.string_to_wstring(lua_tostring(L, -1));
...
std::wstring luaL_checkwstring(lua_State* L, int i)
{
    ...
    return g_io_service.string_to_wstring(str);
}
...
{
    const auto s = g_io_service.wstring_to_string(str);
...
fail:
    if (has_error)
    {
        ...
        const auto error = g_io_service.string_to_wstring(lua_tostring(env->L, -1));
