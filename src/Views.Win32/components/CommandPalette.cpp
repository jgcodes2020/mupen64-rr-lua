...
static bool try_change_hotkey(int32_t i)
{
    ...
    Hotkey::show_prompt(g_main_wnd.main_hwnd, std::format(L"Choose a hotkey for {}", item->text), hotkey);
    Hotkey::try_associate_hotkey(g_main_wnd.main_hwnd, item->path, hotkey);
...
static void build_listbox()
{
    ...
    const auto action_matches_query = [&](const t_listbox_item& item, const std::wstring& query) -> bool {
        ...
        const auto matches = normalized_action.contains(query) || normalized_group_name.contains(query) || normalized_hotkey.contains(query) || normalized_raw_display_name.contains(query);
    ...
    case WM_INITDIALOG:
        {
            ...
            RECT parent_rc{};
            GetClientRect(g_main_wnd.main_hwnd, &parent_rc);
            ...
            MapWindowRect(g_main_wnd.main_hwnd, HWND_DESKTOP, &rc);
...
{
    g_ctx = {};
    const HWND hwnd = CreateDialog(g_main_wnd.app_instance, MAKEINTRESOURCE(IDD_COMMAND_PALETTE), g_main_wnd.main_hwnd, command_palette_proc);
