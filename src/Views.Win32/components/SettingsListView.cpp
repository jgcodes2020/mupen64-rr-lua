...
HWND SettingsListView::create(const t_settings_listview_context& ctx)
{
    ...
    *ctx2 = ctx;
    ...
    HWND lvhwnd = CreateWindowEx(WS_EX_CLIENTEDGE, WC_LISTVIEW, NULL, WS_TABSTOP | WS_VISIBLE | WS_CHILD | LVS_SINGLESEL | LVS_REPORT | LVS_SHOWSELALWAYS | LVS_ALIGNTOP, ctx.rect.left, ctx.rect.top, ctx.rect.right - ctx.rect.left, ctx.rect.bottom - ctx.rect.top, ctx.dlg_hwnd, (HMENU)IDC_SETTINGS_LV, g_main_ctx.app_instance, NULL);
    ...
    ImageList_AddIcon(image_list, LoadIcon(g_main_ctx.app_instance, MAKEINTRESOURCE(IDI_DENY)));
    ImageList_AddIcon(image_list, LoadIcon(g_main_ctx.app_instance, MAKEINTRESOURCE(IDI_CHANGED)));
