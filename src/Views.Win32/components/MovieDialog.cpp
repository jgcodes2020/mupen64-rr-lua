...
refresh:
    ...
    metadata.emplace_back(std::make_pair(L"ROM", std::format(L"{} ({}, {})", g_main_ctx.io_service.string_to_wstring((char*)header.rom_name), g_main_ctx.core_ctx->vr_country_code_to_country_name(header.rom_country), std::format(L"{:#08x}", header.rom_crc1))));
    ...
    metadata.emplace_back(
    std::make_pair(L"Authorship", header.extended_version == 0 ? L"Unknown" : g_main_ctx.io_service.string_to_wstring(authorship)));
...
MovieDialog::t_result MovieDialog::show(bool readonly)
{
    ...
    user_result.path = std::format(L"{} ({}).m64", g_main_ctx.io_service.string_to_wstring((char*)rom_hdr->nom), g_main_ctx.core_ctx->vr_country_code_to_country_name(rom_hdr->Country_code));
