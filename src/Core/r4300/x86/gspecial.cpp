...
void genjr()
{
    ...
    uint32_t diff = (uint32_t)(&dst->local_addr) - (uint32_t)(dst);
    uint32_t diff_need = (uint32_t)(&dst->reg_cache_infos.need_map) - (uint32_t)(dst);
    uint32_t diff_wrap = (uint32_t)(&dst->reg_cache_infos.jump_wrapper) - (uint32_t)(dst);
    uint32_t temp, temp2;
